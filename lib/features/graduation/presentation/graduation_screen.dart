...
class GraduationPartiesScreen extends StatefulWidget {
  ...
  @override
  State<GraduationPartiesScreen> createState() =>
      _GraduationPartiesScreenState();
}
...
class _GraduationPartiesScreenState extends State<GraduationPartiesScreen> {
      ...
      builder: (context) => AppDialog(
        ...
        onConfirm: () async {
          if (titleController.text.isNotEmpty) {
            bool success;
            if (isEditing) {
              success = await _controller.editVideo(
                video.id,
                titleController.text,
                urlController.text,
              );
            } else {
              success = await _controller.addVideo(
                titleController.text,
                urlController.text,
              );
            }
          ...
          if (success) {
            ...
            if (_controller.hasError && !_controller.hasValidationErrors) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    _controller.errorMessage ?? 'Error deleting video',
                  ),
                ),
              );
            }
  ...
  void _openSecurePlayer(String urlString, String title) {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            SecureVideoPlayerScreen(videoUrl: urlString, title: title),
      ),
    );
  }
    ...
    return AppScaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        child: const Icon(
          Icons.add,
          color: Colors.black,
        ), // Fixed Icon: add instead of add_a_photo
      ),
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: Column(
                  // Changed from Stack to Column to allow content to grow naturally below image/placeholder
                    ...
                    Container(
                      ...
                      decoration: const BoxDecoration(
                        image: DecorationImage(
                          image: AssetImage(
                            'assets/images/graduation_party_bg.png',
                          ),
                          fit: BoxFit.cover,
                        ),
                      ),
                        ...
                        children: [
                          Text(
                            ...
                            style: const TextStyle(
                              color: AppColors
                                  .textPrimary, // Adjusted color since it's not on dark overlay anymore
                              fontSize: 20,
